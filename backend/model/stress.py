from sqlalchemy import Column, Integer, Date, DateTime, ForeignKey, Index, CheckConstraint
...
class StressDaily(Base, TimeStampMixin):
    ...
    readings = relationship("StressReading", 
                          back_populates="daily_summary",
                          primaryjoin="and_(StressDaily.user_id==StressReading.user_id, "
                                    "cast(StressDaily.date as DateTime)==cast(StressReading.timestamp as Date))")
...
class StressReading(Base):
    ...
    daily_summary = relationship("StressDaily", 
                               back_populates="readings",
                               foreign_keys=[user_id],
                               primaryjoin="and_(StressDaily.user_id==StressReading.user_id, "
                                         "cast(StressDaily.date as DateTime)==cast(StressReading.timestamp as Date))")

    user = relationship("User", backref="stress_readings")
