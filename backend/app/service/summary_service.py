...
from app.domain import Activity, DailySummary, SleepHRV
...
class GarminSummaryService(BaseGarminService):
    ...
    def get_daily_summary(self, date: str) -> Optional[DailySummary]:
        ...
        try:
            ...
            logger.error(
                "%s 조회 실패 - Date: %s, Error: %s", endpoint_name, date, str(e)
            )
    ...
    def get_activities(
        ...
        try:
            ...
            logger.error(
                "%s 조회 실패 - Limit: %d, Start: %d, Error: %s",
                endpoint_name,
                limit,
                start,
                str(e),
